...
rho_oil = 970;          % kg/m^3, from the uploaded thermal-effect document
eta20 = oil_dynamic_viscosity(20, rho_oil);

records = repmat(empty_record(), 0, 1);
for T = temperatures
    nu_T = oil_kinematic_viscosity(T);
    eta_T = oil_dynamic_viscosity(T, rho_oil);
    alpha_p_T = pressure_viscosity_coefficient(T);
    ...
    tempDir = fullfile(desktopOut, sprintf('oil_%03dC', T));
    ...
    records(end + 1, 1) = run_ball_case(projectRoot, ballData, cfg, ballOut, T, nu_T, eta_T, alpha_p_T, viscosity_factor); %#ok<SAGROW>
    ...
    records(end + 1, 1) = run_roller_case(projectRoot, rollerData, cfg, rollerOut, T, nu_T, eta_T, alpha_p_T, viscosity_factor); %#ok<SAGROW>

    writetable(struct2table(records), fullfile(desktopOut, 'oil_temperature_summary.csv'), 'Encoding', 'UTF-8');
end

writetable(struct2table(records), fullfile(desktopOut, 'oil_temperature_summary.csv'), 'Encoding', 'UTF-8');
disp('OIL_TEMPERATURE_CASES_DONE');
disp(desktopOut);
...
function record = run_ball_case(projectRoot, datafromvb, cfg, outDir, T, nu_T, eta_T, alpha_p_T, viscosity_factor)
oldDir = pwd;
cleanupObj = onCleanup(@() cd(oldDir));
cd(fullfile(projectRoot, '球轴承程序'));
record = empty_record();
record.bearing_type = 'ball';
record.oil_temperature_C = T;
...
function record = run_roller_case(projectRoot, datafromvb, cfg, outDir, T, nu_T, eta_T, alpha_p_T, viscosity_factor)
oldDir = pwd;
cleanupObj = onCleanup(@() cd(oldDir));
